...
async function seedCategories() {
    try {
        logger_1.logger.info('开始初始化分类数据...');
        for (const categoryData of categories) {
            ...
            if (!existingCategory) {
                ...
                logger_1.logger.info(`创建分类: ${categoryData.name}`);
            }
        }
        logger_1.logger.info('分类数据初始化完成');
    }
    catch (error) {
        logger_1.logger.error('初始化分类数据失败:', error);
        throw error;
    }
}
async function seedProducts() {
    try {
        logger_1.logger.info('开始初始化商品数据...');
        for (const productData of products) {
            const category = await Category_1.default.findOne({
                where: { name: productData.categoryName }
            });
            if (!category) {
                logger_1.logger.warn(`分类不存在: ${productData.categoryName}`);
                continue;
            }
            ...
            if (!existingProduct) {
                ...
                logger_1.logger.info(`创建商品: ${productData.name}`);
            }
        }
        logger_1.logger.info('商品数据初始化完成');
    }
    catch (error) {
        logger_1.logger.error('初始化商品数据失败:', error);
        throw error;
    }
}
async function seedAllData() {
    try {
        logger_1.logger.info('开始初始化商城数据...');
        await seedCategories();
        await seedProducts();
        logger_1.logger.info('商城数据初始化完成');
    }
    catch (error) {
        logger_1.logger.error('初始化商城数据失败:', error);
        throw error;
    }
}
