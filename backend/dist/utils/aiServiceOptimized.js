...
exports.OptimizedAIService = void 0;
...
const conversationContexts = new LRUCache(1000);
const keywordCache = new LRUCache(500);
const categoryCache = new LRUCache(500);
const knowledgeCache = new LRUCache(200);
...
class OptimizedAIService {
    static async processQuestion(question, userId) {
        ...
        try {
            ...
            const processingTime = performance.now() - startTime;
            logger_1.logger.error('AI服务处理失败', {
                error: error.message,
                processingTime: `${processingTime.toFixed(2)}ms`,
                timestamp: new Date().toISOString()
            });
    ...
    static preprocessQuestionOptimized(question) {
    ...
    static extractKeywordsOptimized(question) {
    ...
    static async searchKnowledgeBaseOptimized(keywords, category) {
        try {
            ...
            const conditions = keywords.map(keyword => ({
                [sequelize_1.Op.or]: [
                    ...
                    { keywords: { [sequelize_1.Op.contains]: [keyword] } },
                    { tags: { [sequelize_1.Op.contains]: [keyword] } },
                ],
            }));
            ...
            const results = await CustomerService_1.KnowledgeBase.findAll({
            ...
            if (results.length === 0) {
                ...
                const fuzzyResults = await CustomerService_1.KnowledgeBase.findAll({
                    where: {
                        isActive: true,
                        [sequelize_1.Op.or]: fuzzyConditions,
                    },
                    order: [['updatedAt', 'DESC']],
                    limit: 5,
                });
                const result = fuzzyResults.map((result) => result.toJSON());
            ...
            logger_1.logger.error('搜索知识库失败:', error);
            return [];
        }
    }
    static classifyQuestionOptimized(question) {
        const cacheKey = question.substring(0, 50);
    ...
    static classifyQuestionTypeOptimized(question) {
    ...
    static async generateAnswerOptimized(question, knowledgeResults, category, questionType) {
    ...
    static calculateConfidenceOptimized(keywords, knowledgeResults, category, questionType) {
        if (knowledgeResults.length === 0) {
            return 0.3;
        }
        let confidence = 0;
    ...
    static generateSuggestionsOptimized(category, questionType) {
        const suggestions = {
            '订单': {
    ...
    static analyzeUserIntentOptimized(question, context) {
    ...
    static determineConversationFlowOptimized(question, context, intent) {
    ...
    static adjustAnswerBasedOnIntentOptimized(answer, intent, context) {
        switch (intent) {
            case 'clarification':
    ...
    static adjustSuggestionsBasedOnFlowOptimized(suggestions, flow, context) {
        switch (flow) {
    ...
    static updateContextMemoryOptimized(context, question, result, intent) {
    ...
    static updatePerformanceMetrics(context, processingTime) {
    ...
    static cleanupOldMessagesOptimized(context) {
        const recentMessages = context.messages.slice(-10);
        const importantMessages = context.messages.filter(msg => msg.intent === 'complaint' ||
            msg.intent === 'urgent' ||
            (msg.confidence && msg.confidence > 0.8));
        const allImportantMessages = [...importantMessages, ...recentMessages];
        const uniqueMessages = allImportantMessages.filter((msg, index, arr) => arr.findIndex(m => m.content === msg.content) === index);
        context.messages = uniqueMessages.slice(-20);
    }
    static getDetailedExplanationOptimized(topic) {
        const explanations = {
    ...
    static getSimpleAnswerOptimized(answer, topic) {
        const simpleAnswers = {
            '订单': '订单状态可在"我的订单"页面查看。',
            '退款': '退款申请后1-3个工作日处理完成。',
            '物流': '物流信息在订单详情页面查看。',
    ...
    static enhanceQuestionWithContextOptimized(question, context) {
    ...
    static clearConversationContext(sessionId) {
        conversationContexts.set(sessionId, null);
    }
    ...
    static clearCaches() {
        ...
        logger_1.logger.info('AI服务缓存已清理');
    }
...
exports.OptimizedAIService = OptimizedAIService;
exports.default = OptimizedAIService;
//# sourceMappingURL=aiServiceOptimized.js.map