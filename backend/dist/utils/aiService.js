...
class AIService {
    ...
    static async processConversation(question, sessionId, userId) {
        try {
    ...
    static preprocessQuestion(question) {
    ...
    static async searchKnowledgeBase(keywords, category) {
        try {
            ...
            if (results.length === 0) {
                ...
                return fuzzyResults.map((result) => result.toJSON());
            }
    ...
    static classifyQuestionType(question) {
    ...
    static async generateAnswer(question, knowledgeResults, category, questionType) {
    ...
    static calculateConfidence(keywords, knowledgeResults, category, questionType) {
        if (knowledgeResults.length === 0) {
            return 0.3;
        }
        let confidence = 0;
    ...
    static generateSuggestions(category, questionType) {
        const suggestions = {
            '订单': {
    ...
    static analyzeUserIntent(question, context) {
    ...
    static determineConversationFlow(question, context, intent) {
    ...
    static adjustAnswerBasedOnIntent(answer, intent, context) {
        switch (intent) {
            case 'clarification':
    ...
    static adjustSuggestionsBasedOnFlow(suggestions, flow, context) {
        switch (flow) {
    ...
    static updateContextMemory(context, question, result, intent) {
    ...
    static cleanupOldMessages(context) {
    ...
    static getDetailedExplanation(topic) {
        const explanations = {
    ...
    static getSimpleAnswer(answer, topic) {
        const simpleAnswers = {
            '订单': '订单状态可在"我的订单"页面查看。',
            '退款': '退款申请后1-3个工作日处理完成。',
            '物流': '物流信息在订单详情页面查看。',
    ...
    static enhanceQuestionWithContext(question, context) {
    ...
    static async addKnowledgeEntry(data) {
        try {
            return await CustomerService_1.KnowledgeBase.create({
                ...data,
    ...
    static async updateKnowledgeEntry(id, data) {
        try {
            const entry = await CustomerService_1.KnowledgeBase.findByPk(id);
            if (!entry) {
                return null;
            }
    ...
    static async deleteKnowledgeEntry(id) {
        try {
            const entry = await CustomerService_1.KnowledgeBase.findByPk(id);
            if (!entry) {
                return false;
    ...
    static async getKnowledgeStats() {
        try {
            const total = await CustomerService_1.KnowledgeBase.count();
            const active = await CustomerService_1.KnowledgeBase.count({ where: { isActive: true } });
